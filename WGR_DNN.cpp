...
MatrixXd ReLU(MatrixXd A){
  for(int i=0; i<A.rows(); ++i){
    for(int j=0; j<A.cols(); ++j){
      // Leaky ReLU
      //if(A(i,j)<0.0){A(i,j)=A(i,j)/100.0;}
      // Regular ReLU
...
SEXP FNN(MatrixXd y, // Phenotype
  ...
  for(int i=0; i<epochs; ++i){
    ...
    dH1 = Dropout(dH1,do1);
    // Compute gradient
    for(int j=0; j<HL1; ++j){dW1.col(j) = CholX.solve(X.transpose()*dH1.col(j));}
    dI1 = H1.colwise().mean();
    // Update 
    ...
    dH2 = Dropout(dH2,do2);
    // Factorize
    HpH1 = H1.transpose() * H1;
    for(int j=0; j<HL1; ++j){ HpH1(j,j) = HpH1(j,j)+lmb2; }
    // Compute gradient
    dW2 = HpH1.llt().solve(H1.transpose()*dH2);
    dI2 = H2.colwise().mean();
    // Update 
